...
BEGIN
{
  ...
  $postmaster        = '';
  #@referers          = qw(rhymer2.securesites.net);
  @referers          = qw(scripturecreations.com www.scripturecreations.com rhymer2.securesites.net localhost);
  @allow_mail_to     = qw(writeexpress@yahoo.com);
  @recipients        = ();
  %recipient_alias   = (1 => q(writeexpress@yahoo.com));
...
=head1 SYNOPSYS
  ...
  my $mailer = CGI::NMS::Mailer::Sendmail->new('/usr/sbin/sendmail -oi -t');
...
# Tack on: encryption
sub new_crypt {
  my ($obj, $encryptuser, $encryptprog) = @_;
  if (! $encryptuser or ! $encryptprog) { return }
  $obj->{Encryptuser} = $encryptuser;
  $obj->{Encryptprog} = $encryptprog;
}
...
sub newmail {
  ...
  # Tack on: setup encryption pipe

  if (! $self->{Encryptprog}) { return }

  my ($encr_fh, $encr_file) = tempfile("fmencrXXXX", DIR => "/tmp", OPEN => 0);
  $self->{Encryptfile} = $encr_file;
  $command = $self->{Encryptprog} . $self->{Encryptuser} . " > $encr_file";
  ...
  die "Can't open encryptprog [$command]\n" unless $pipe;
  $self->{CryptPipe} = $pipe;
}
...
sub encrypt {
  my ($self, @args) = @_;
  $self->{CryptPipe}->print(@args) or die "write to encryption pipe: $!";
}
...
sub endmail {
  ...
  if ($self->{Encryptprog}) {
    $self->{CryptPipe}->close or die "Can't close CryptPipe $!";
    open(CRYP, $self->{Encryptfile}) || die "Can't re-open $self->{Encryptfile} $!";
    while (<CRYP>) { $self->print($_) }
  }
...
=head1 SYNOPSYS
  ...
  my $mailer = CGI::NMS::Mailer::ByScheme->new('/usr/sbin/sendmail -oi -t');
...
Default: C<'/usr/sbin/sendmail -oi -t'>
...
addresses hidden.  You might set up C<recipient_alias> like this:
  ...
  %recipient_alias   = (1 => q(Support@WriteExpress.com));
    '1' => 'one@your.domain',
    '2' => 'two@your.domain',
    '3' => 'three@your.domain',
  );
...
sub init {
  ...
  $self->{Mailer}->new_crypt($self->{CFG}{encryptuser}, $self->{CFG}{encryptprog});
}
...
sub send_main_email_header {
  ...
  my $subject = $self->{FormConfig}{subject} || 'BoomStart Support Request';
  if ($self->{CFG}{secure}) {
  ...
  if ($self->{FormConfig}{print_config}) {
    foreach my $cfg (@{ $self->{FormConfig}{print_config} }) {
      if ($self->{FormConfig}{$cfg}) {
        $self->print_body_part("$cfg: $self->{FormConfig}{$cfg}\n");
	$self->print_body_part("\n") if $self->{CFG}{double_spacing};
      }
    }
  }
}
...
sub send_main_email_field {
  ...
  if ($self->{CFG}{wrap_text} and length("$prefix$line") > $self->email_wrap_columns) {
    $self->print_body_part( $self->wrap_field_for_email($prefix, $line) . $nl );
  }
  else {
    $self->print_body_part("$prefix$line$nl");
  }
}
...
sub send_main_email_footer {
  ...
  my $dashes = '-' x 75;
  $self->print_body_part("$dashes\n\n");

  foreach my $e (@{ $self->{FormConfig}{env_report}}) {
    if ($ENV{$e}) {
      $self->print_body_part("$e: " . $self->strip_nonprint($ENV{$e}) . "\n");
    }
    }
}
...
sub print_body_part {
  ...
  if ($self->{Mailer}->{CryptPipe}) {
    $self->{Mailer}->encrypt(@args);
  }
  else {
    $self->mailer->print(@args);
  }
}
